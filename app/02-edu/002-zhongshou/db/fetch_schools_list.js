      ...
      for (const school of schools) {
        ...
        db.get('SELECT id FROM schools WHERE school_id = ?', [school.school_id], (err, row) => {
          ...
          if (row) {
            ...
            db.run(
              'UPDATE schools SET name = ?, deviation = ?, sex = ? WHERE id = ?',
              [school.name, school.deviation, school.sex, row.id],
              (err) => {
                if (err) {
            ...
            db.run(
              'INSERT OR REPLACE INTO schools (school_id, name, deviation, sex) VALUES (?, ?, ?, ?)',
              [school.school_id, school.name, school.deviation, school.sex],
              function (err) {
                if (err) {
